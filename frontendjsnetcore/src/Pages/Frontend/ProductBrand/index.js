...
function ProductBrand() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const result_brand = await BrandService.getById(id);
        const brid = result_brand.data.id;
        setTitle(result_brand.data.name);
        const result = await ProductServices.getProductByBrandId(
          brid,
          limit,
          page
        );
